...
def main(path_temp_dummy: str, path_temp: str):
    print("[PromptDA] started upscaling depth")
    
    files_color = sorted(glob.glob(os.path.join(path_temp, 'rgb', '*.png')))
    files_depth = sorted(glob.glob(os.path.join(path_temp, 'depth', '*.png')))
    ...
    for frame_idx, (curr_color, curr_depth) in tqdm(enumerate(zip(color, depth)), desc='Inferring', total=len(color)):
        ...
        save_depth(pred_depth.detach().cpu(), output_path=os.path.join(path_temp, 'depth_upscaled', os.path.basename(files_depth[frame_idx])), save_vis=False)
